...
-(void)setShowEnglishText:(NSString *)showEnglishText
{
    ...
    //背景色
    showEnglishLabel.backgroundColor=[UIColor clearColor];
    //文本赋值
    ...
    //按钮圆角
    translateButton.layer.cornerRadius =kCornerRadiusOfButton;

    //设置字符
    [translateButton setTitle:@"翻译中文" forState:UIControlStateNormal];
    
   //透明度                              
    translateButton.backgroundColor = [[UIColor blackColor]colorWithAlphaComponent:kBackgroundAlphaValueOfTranslateButton];;
    
    //设置边框宽度    
    [translateButton.layer setBorderWidth:kBorderWidthOfTranslateButton ];
    
     //背景颜色透明。字体不透明
    translateButton.layer.borderColor=[UIColor blackColor].CGColor;
